...
import { useContentMode } from "@/features/content-editor/hooks/useContentMode";
import { ContentModeToggle } from "@/features/content-editor/components/ContentModeToggle";
import { ContentModeLayout } from "@/features/content-editor/components/ContentModeLayout";
...
export default function DomainEditorPage() {
  ...
  const onContentDeletePage = async (file: EditorFileMeta) => {
    if (readOnly) return;
    try {
      await deleteFile(domainId, file.path);
      if (selection?.selectedPath === file.path) {
        setSelection(null);
        setDirtyState({ isDirty: false, originalContent: "", currentContent: "" });
      }
      await loadFiles();
      await loadDeletedFiles();
      showToast({ type: "success", title: "Страница удалена", message: file.path });
    } catch (err: any) {
      showToast({ type: "error", title: "Не удалось удалить страницу", message: err?.message || "unknown error" });
    }
  };

  const onContentRestorePage = async (file: EditorFileMeta) => {
    try {
      ...
      showToast({ type: "success", title: "Страница восстановлена", message: file.path });
    } catch (err: any) {
      showToast({ type: "error", title: "Не удалось восстановить страницу", message: err?.message || "unknown error" });
    }
  };
      ...
      {error && (
        ...
        <section className="space-y-3">
          ...
          <ContentModeLayout
            ...
            onLoadFile={loadFile}
            onDeletePage={onContentDeletePage}
            onRestorePage={onContentRestorePage}
            historyRefreshKey={historyRefreshKey}
            onHistoryReverted={async () => {
              setHistoryRefreshKey((v) => v + 1);
              const refreshed = await loadFiles();
              const sp = selection?.selectedPath;
              if (sp) {
                const file = refreshed.find((f) => f.path === sp);
