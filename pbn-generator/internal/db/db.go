...
func migrationStatements() []string {
	...
	projectStmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS site_files (
			...
			last_edited_by TEXT REFERENCES users(email),
			deleted_at TIMESTAMPTZ,
			deleted_by TEXT REFERENCES users(email),
			delete_reason TEXT,
			created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
		);`,
		`ALTER TABLE site_files ADD COLUMN IF NOT EXISTS version INT NOT NULL DEFAULT 1;`,
		`ALTER TABLE site_files ADD COLUMN IF NOT EXISTS last_edited_by TEXT REFERENCES users(email);`,
		`ALTER TABLE site_files ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ;`,
		`ALTER TABLE site_files ADD COLUMN IF NOT EXISTS deleted_by TEXT REFERENCES users(email);`,
		`ALTER TABLE site_files ADD COLUMN IF NOT EXISTS delete_reason TEXT;`,
		`ALTER TABLE site_files DROP CONSTRAINT IF EXISTS site_files_domain_id_path_key;`,
		`CREATE INDEX IF NOT EXISTS idx_site_files_domain ON site_files(domain_id);`,
		`CREATE INDEX IF NOT EXISTS idx_site_files_deleted ON site_files(domain_id, deleted_at);`,
		`CREATE UNIQUE INDEX IF NOT EXISTS idx_site_files_domain_path_active ON site_files(domain_id, path) WHERE deleted_at IS NULL;`,
		...
		`CREATE INDEX IF NOT EXISTS idx_llm_model_pricing_model ON llm_model_pricing(provider, model, active_from DESC);`,
		`CREATE UNIQUE INDEX IF NOT EXISTS idx_llm_model_pricing_active ON llm_model_pricing(provider, model) WHERE is_active = TRUE;`,
		...
		`CREATE INDEX IF NOT EXISTS idx_llm_usage_events_requester ON llm_usage_events(requester_email);`,
		`CREATE INDEX IF NOT EXISTS idx_llm_usage_events_project ON llm_usage_events(project_id);`,
		`CREATE INDEX IF NOT EXISTS idx_llm_usage_events_domain ON llm_usage_events(domain_id);`,
		`CREATE INDEX IF NOT EXISTS idx_llm_usage_events_generation ON llm_usage_events(generation_id);`,
		`CREATE INDEX IF NOT EXISTS idx_llm_usage_events_model ON llm_usage_events(model);`,
		`CREATE INDEX IF NOT EXISTS idx_llm_usage_events_operation ON llm_usage_events(operation);`,
		...
		// --- Soft-delete for projects and domains ---
