...
func main() {
	...
	mode := flag.String("mode", "", "backfill mode: dry-run|apply")
	...
	reportPath := flag.String("report", "backfill_legacy_artifacts_report.json", "output path for JSON report")
	...
	logger.Infow("backfill finished",
		"mode", report.Mode,
	...
	fmt.Printf("backfill completed: processed=%d success=%d warned=%d failed=%d decoded=%d updated=%d skipped=%d unchanged=%d report=%s\n",
